      ...
      <>
        ...
        {anagramHasBeenStarted ? (
          <DndProvider backend={HTML5Backend}>
            <CurrentWords words={currentWords} onAdd={onAdd} onRemove={onRemove} />
            <div>
              <p>Remaining Letters</p>
              {availableLetters.map((letter, index) => (
                <span className={styles.remainingLetters} key={`${letter}${index}`}>
                  {letter}
                </span>
              ))}
            </div>
...
const initialize_dictionary = async (mutableRefObject) => {
  try {
    ...
    let dictionary_file_path = `/text/${encodeURIComponent('Collins Scrabble Words (2019).txt')}`;
    let dictionary = await (await fetch(dictionary_file_path)).text();
    console.log(
      `Successfully fetched dictionary file '${dictionary.slice(0, 29)}', loading into wasm...`
    );
