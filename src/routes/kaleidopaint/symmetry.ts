...
export function drawMosaicPreview(
	...
	for (const cell of cells) {
		let r = lineLen;
		if (cell.folds === 3) r = T / sqrt3; // Triangle circumradius for edge-to-edge
		else if (cell.folds === 6) r = T * sqrt3 / 2; // Hex inradius
		const triRot = cell.folds === 3 && cell.flip
...
export function getSymmetricPoints(
	...
	if (mode === 'mosaic' && canvasWidth != null && canvasHeight != null && mosaicTypeVal) {
		return getMosaicPoints(x, y, centerX, centerY, mosaicTypeVal, rotationDeg, canvasWidth, canvasHeight);
	}
...
function rotatePoint(px: number, py: number, cx: number, cy: number, rad: number): [number, number] {
	const dx = px - cx;
	const dy = py - cy;
