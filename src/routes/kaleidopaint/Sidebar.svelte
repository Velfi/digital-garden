<script lang="ts">
  ...
  const BRUSH_TIPS: Record<string, string> = {
    ...
    spacing: 'Distance between brush dabs as % of brush size. Lower = smoother strokes, higher = more dotted.',
    ...
    rotation: 'How the brush rotates: fixed angle, radially from symmetry origin, or along the stroke direction.',
    'angle-rotation': 'Fixed rotation in degrees when using fixed rotation mode.',
    'isotropic-spacing': 'Use same spacing in all directions. When off, ellipse ratio affects spacing along the stroke.',
  ...
  function drawBrushPreviewToCanvas() {
    ...
    if ($brushRotationMode === 'origin' || $brushRotationMode === 'drawing') {
      (previewParams as BrushParams & { previewRotationRad?: number }).previewRotationRad = Math.PI / 4;
    }
    drawBrushPreview(ctx, previewParams, 'rgba(0,0,0,0.5)');
    ctx.restore();
  }

  $: $brushSize, $brushShape, $brushAngle, $brushRatio, $color, $brushRotationMode, $brushRotationAngle, $brushMirrorH, $brushMirrorV, $tool, (() => {
  ...
  async function loadPaletteBySlug(slug: string) {
    ...
    try {
      ...
      const hexColors = (data.colors ?? []).map((c: string) =>
        c.startsWith('#') ? c : `#${c}`
      );
...
<div class="sidebar-wrapper">
  ...
  <label class="brush-param">
    ...
    <input type="range" min="10" max="500" value={$brushSpacing * 100} on:input={(e) => brushSpacing.set(Number((e.target as HTMLInputElement).value) / 100)} />
  ...
  {#if $brushShape === 'ellipse' || $brushShape === 'rectangle'}
    ...
    <label class="brush-param">
      ...
      <input type="range" min="10" max="100" value={$brushRatio * 100} on:input={(e) => brushRatio.set(Number((e.target as HTMLInputElement).value) / 100)} />
  ...
  <label class="brush-param">
    ...
    <input type="range" min="0" max="100" value={$brushOpacity * 100} on:input={(e) => brushOpacity.set(Number((e.target as HTMLInputElement).value) / 100)} />
    <span class="value">{Math.round($brushOpacity * 100)}%</span>
  </label>
  <label class="brush-param">
    <ParamLabel label="Flow" tip={BRUSH_TIPS.flow} id="tip-flow" />
    <input type="range" min="0" max="100" value={$brushFlow * 100} on:input={(e) => brushFlow.set(Number((e.target as HTMLInputElement).value) / 100)} />
    <span class="value">{Math.round($brushFlow * 100)}%</span>
  </label>
  <label class="brush-param">
    <ParamLabel label="Softness" tip={BRUSH_TIPS.softness} id="tip-softness" />
    <input type="range" min="0" max="100" value={$brushSoftness * 100} on:input={(e) => brushSoftness.set(Number((e.target as HTMLInputElement).value) / 100)} />
    <span class="value">{Math.round($brushSoftness * 100)}%</span>
  </label>
  <label class="brush-param">
    <ParamLabel label="Sharpness" tip={BRUSH_TIPS.sharpness} id="tip-sharpness" />
    <input type="range" min="0" max="100" value={$brushSharpness * 100} on:input={(e) => brushSharpness.set(Number((e.target as HTMLInputElement).value) / 100)} />
  ...
  <div class="scatter-mirror-row">
    <div class="scatter-section">
      ...
      <label class="brush-param">
        ...
        <input type="range" min="0" max="100" value={$brushScatterX * 100} on:input={(e) => brushScatterX.set(Number((e.target as HTMLInputElement).value) / 100)} />
      </label>
      <label class="brush-param">
        <ParamLabel label="Y" tip={BRUSH_TIPS['scatter-y']} id="tip-scatter-y" />
        <input type="range" min="0" max="100" value={$brushScatterY * 100} on:input={(e) => brushScatterY.set(Number((e.target as HTMLInputElement).value) / 100)} />
      </label>
    </div>
  ...
  <label class="toggle">
    ...
    <ParamLabel label="Isotropic spacing" tip={BRUSH_TIPS['isotropic-spacing']} id="tip-isotropic" />
  </label>
  ...
  <div class="brush-params-grid">
    ...
    {#if $brushSource === 'plain'}
      <label class="brush-param">
        ...
        <input type="range" min="0" max="100" value={$brushMix * 100} on:input={(e) => brushMix.set(Number((e.target as HTMLInputElement).value) / 100)} />
  ...
  <p class="lospec-hint">
    <a href="https://lospec.com/palette-list" target="_blank" rel="noopener">Browse palettes</a> and enter a slug (e.g. greyt-bit, apollo)
  </p>
  ...
  <div class="symmetry-section">
    ...
    {#if $symmetryEnabled && ($symmetryMode === 'linear' || $symmetryMode === 'polar' || $symmetryMode === 'mosaic')}
      <label class="brush-param">
        <ParamLabel label="Rotation" tip="Angle of the symmetry axis (linear) or first fold (polar)." id="tip-symmetry-rotation" />
        <input type="range" min="0" max="360" bind:value={$symmetryRotation} />
        <span class="value">{$symmetryRotation}°</span>
      </label>
      <button type="button" on:click={() => { symmetryOriginX.set(0.5); symmetryOriginY.set(0.5); }} title="Reset symmetry origin to center">
        Center origin
      </button>
    {/if}
  ...
  <h2>Canvas</h2>
  <div class="undo-redo">
    <button type="button" disabled={!$canUndo} on:click={() => history.undo()} title="Undo (Ctrl+Z / Cmd+Z)">Undo</button>
    <button type="button" disabled={!$canRedo} on:click={() => history.redo()} title="Redo (Ctrl+Shift+Z / Cmd+Y)">Redo</button>
  </div>
