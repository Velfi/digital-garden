...
function makeVoxels(entries: [string, number][]) {
  return new Map(entries);
}

describe('createUndo', () => {
  it('pushUndo snapshots state and doUndo restores', () => {
    const voxels = writable(makeVoxels([['0,0,0', 0xff0000]]));
    const selection = writable(new Map<string, number>());
    const undo = createUndo(voxels, selection);

    undo.pushUndo();
    voxels.set(makeVoxels([['1,1,1', 0x00ff00]]));
    selection.set(makeVoxels([['1,1,1', 0x00ff00]]));
