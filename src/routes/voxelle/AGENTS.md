...
- **`canvas/voxelRayDda.ts`**, **`canvas/voxelRayTsl.ts`**, **`canvas/voxelRayGpuResources.ts`**, **`canvas/voxelRayProgressive.ts`** – **`ray`** mode (selectable in the UI only when WebGPU is active; WebGL falls back to `greedy`): primary **shading** is still **CPU progressive** ray marching in `VoxelRayProgressive`; `VoxelRayTsl` rebuilds **GPU voxel acceleration** (dense `Data3DTexture` or hash) on invalidate for upcoming parity / uploads. Tool **picking** uses CPU DDA on the voxel `Map` (same as mesh modes’ spatial queries). Shadow rays, Fresnel + Beer–Lambert glass/water, and glow bloom sources match the progressive tracer. `VoxelCanvas` feeds ray beauty + bloom textures into the same selective bloom pipeline as mesh mode.
