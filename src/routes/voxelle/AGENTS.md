...
- **`canvas/handlers/`** – Pointer dispatch. `pointerHandler.ts`: overlay passthrough, `fly.ts`, optional **`generatorPointer.ts`** (RMB reseed / ashlar preview / gesture cancel via `GeneratorRmbDeps`), `handleFlyPointerUp`. Face-click placement on pointer up: `applyGeneratorFaceClickPointerUp`. VoxelCanvas builds deps objects; extend `types.ts` / new sidecar contexts as more tools move out of the component.
