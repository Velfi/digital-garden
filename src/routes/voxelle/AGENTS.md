...
- **`canvas/voxelRayDda.ts`**, **`canvas/voxelRayProgressive.ts`** – **`ray`** mode: CPU grid DDA + progressive refinement into a `DataTexture` scene background (no greedy mesh). **Ray shading:** shadow rays (`traceShadowRayDda`, glass-tinted), glass transmission (Fresnel + Beer–Lambert, up to 4 layers), glow emissive. **`VoxelRayProgressive.bloomTexture`:** float `DataTexture` for selective bloom; `VoxelCanvas` runs the same WebGL/WebGPU bloom pipeline as greedy mode with background swap (no mesh stash).
